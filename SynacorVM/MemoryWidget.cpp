	...
	, rawMemory(c_dwAddressSpace)
{
	...
	for (int address = 0; address < rawMemory.size(); address += VALUES_PER_LINE)
	{
		memory << GetMemoryRowAt(rawMemory, address);
	}
...
void MemoryWidget::update()
{
	...
	if (pendingMemoryUpdates.size() > 0)
	{
